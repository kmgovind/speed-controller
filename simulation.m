...
chargeUsage = 0; % track how much battery gets used
...
while(~endRun)
    % TODO: Plotting
    [runVehicle, chargeUsage] = runVehicle.moveBoat(runDomain, goal.lat(legCounter), goal.long(legCounter), currentTime, chargeUsage, legCounter);
    if legCounter == 1
        [endRun, currentTime, legCounter] = endCheck(currentTime, runVehicle, runDomain, goal, wpThresh, legCounter);
        if legCounter == 2
            legOneTime = currentTime;
            legOneCharge = chargeUsage;
        end
    else
        [endRun, currentTime, legCounter] = endCheck(currentTime, runVehicle, runDomain, goal, wpThresh, legCounter);
    end
...
fprintf(fileID, 'Total Charge Usage: %0.2f Watts\n', chargeUsage);
...
fprintf(fileID, 'Leg One Charge Usage: %0.2f Watts\n', legOneCharge);
legTwoTime = currentTime - legOneTime;
hour = hours(minutes(legTwoTime));
minute = (hour - floor(hour)) * 60;
fprintf(fileID, 'Leg 2 Duration:\n\tHours: %d\tMinutes: %d\n', floor(hour), round(minute));
fprintf(fileID, 'Leg Two Charge Usage: %0.2f Watts\n', chargeUsage - legOneCharge);
fclose(fileID);